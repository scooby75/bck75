...
def tips_page():
    ...
    if session_state.user_profile < 1:
        ...
        with tab1:
            ...
            eventos_raros_df = df[(df["FT_Odd_H"] >= 2) & (df["FT_Odd_H"] <= 5) & (df["FT_Odd_Over25"] >= 2.30) & (df["Odd_BTTS_Yes"] <= 2)]
            colunas_desejadas = ["Date", "Time", "League", "Home", "Away"]
            ...
            eventos_raros2_df = df[(df["FT_Odd_A"] >= 2) & (df["FT_Odd_A"] <= 5) & (df["FT_Odd_Over25"] >= 2.30) & (df["Odd_BTTS_Yes"] >= 2)]
