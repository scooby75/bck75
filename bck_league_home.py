...
# Chamar a função para carregar os dados
        top_league_home_df = load_base()

# Filtros interativos
        st.header("Filtros")

        all_leagues = "Todos"
        selected_leagues = st.multiselect("Selecionar Liga(s)", [all_leagues] + list(top_league_home_df['League'].unique()), key="selected_leagues")

        all_rounds = "Todos"
        selected_rounds = st.multiselect("Selecionar Rodada(s)", [all_rounds] + list(top_league_home_df['Round'].unique()), key="selected_rounds")

        all_seasons = "Todos"
        selected_seasons = st.multiselect("Selecionar Temporada(s)", [all_seasons] + list(top_league_home_df['Season'].unique()), key="selected_seasons")

        home_teams = top_league_home_df['Home'].unique()  # Get unique teams from 'Home' column
        selected_home = st.multiselect("Selecionar Mandante", home_teams, key="selected_home")
...
# Apply filters
        filtered_df = top_league_home_df[
            (top_league_home_df['League'].isin(selected_leagues) if all_leagues not in selected_leagues else True) &
            (top_league_home_df['Season'].isin(selected_seasons) if all_seasons not in selected_seasons else True) &
            (top_league_home_df['Round'].isin(selected_rounds) if all_rounds not in selected_rounds else True) &
            (top_league_home_df['Home'].isin(selected_home) if selected_home else True) &
            (top_league_home_df['FT_Odd_H'] >= odd_h_min) &
            (top_league_home_df['FT_Odd_H'] <= odd_h_max) &
            (top_league_home_df['FT_Odd_A'] >= odd_a_min) &
            (top_league_home_df['FT_Odd_A'] <= odd_a_max) &
            (top_league_home_df['FT_Odd_D'] >= odd_draw_min) &
            (top_league_home_df['FT_Odd_D'] <= odd_draw_max) &
            (top_league_home_df['HT_Odd_Over05'] >= over_05ht_min) &
            (top_league_home_df['HT_Odd_Over05'] <= over_05ht_max) &
            (top_league_home_df['FT_Odd_Over25'] >= over_25ft_min) &
            (top_league_home_df['FT_Odd_Over25'] <= over_25ft_max) &
            (top_league_home_df['Odd_BTTS_Yes'] >= btts_yes_min) &
            (top_league_home_df['Odd_BTTS_Yes'] <= btts_yes_max)
        ]
