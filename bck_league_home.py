...
####################################################        
    ...
    #####################################################
        ...
        # Calcula o lucro total das apostas em casa agrupadas por liga e temporada
        profit_away_by_league_season = bck_home_df.groupby(['League', 'Season'])['profit_away'].sum()

        # Filtra as ligas que lucraram pelo menos 1 unidade em todas as temporadas
        profitable_leagues = profit_away_by_league_season.groupby('League').filter(lambda x: (x >= 2).all())

        # Converte o resultado filtrado em um DataFrame
        bck_home_df = profitable_leagues.reset_index()

        # Cria uma tabela dinâmica para organizar os dados
        pivot_table = bck_home_df.pivot_table(index='League', columns='Season', values='profit_away', aggfunc='sum')
     ...
     #####################################################
        ...
        # Calcula o lucro total das apostas em casa agrupadas por liga e temporada
        profit_draw_by_league_season = filtered_df.groupby(['League', 'Season'])['profit_draw'].sum()

        # Filtra as ligas que lucraram pelo menos 1 unidade em todas as temporadas
        profitable_leagues = profit_draw_by_league_season.groupby('League').filter(lambda x: (x >= 2).all())

        # Converte o resultado filtrado em um DataFrame
        filtered_df = profitable_leagues.reset_index()

        # Cria uma tabela dinâmica para organizar os dados
        pivot_table = filtered_df.pivot_table(index='League', columns='Season', values='profit_draw', aggfunc='sum')
     ...
     #####################################################
        ...
        # Calcula o lucro total das apostas em casa agrupadas por liga e temporada
        profit_ov05ht_by_league_season = filtered_df.groupby(['League', 'Season'])['profit_over05HT'].sum()

        # Filtra as ligas que lucraram pelo menos 1 unidade em todas as temporadas
        profitable_leagues = profit_ov05ht_by_league_season.groupby('League').filter(lambda x: (x >= 2).all())

        # Converte o resultado filtrado em um DataFrame
        filtered_df = profitable_leagues.reset_index()

        # Cria uma tabela dinâmica para organizar os dados
        pivot_table = filtered_df.pivot_table(index='League', columns='Season', values='profit_over05HT', aggfunc='sum')
     ...
     #####################################################
        ...
        # Calcula o lucro total das apostas em casa agrupadas por liga e temporada
        profit_u05ht_by_league_season = filtered_df.groupby(['League', 'Season'])['profit_under05HT'].sum()

        # Filtra as ligas que lucraram pelo menos 1 unidade em todas as temporadas
        profitable_leagues = profit_u05ht_by_league_season.groupby('League').filter(lambda x: (x >= 2).all())

        # Converte o resultado filtrado em um DataFrame
        filtered_df = profitable_leagues.reset_index()

        # Cria uma tabela dinâmica para organizar os dados
        pivot_table = filtered_df.pivot_table(index='League', columns='Season', values='profit_under05HT', aggfunc='sum')
      ...
      #####################################################
        ...
        # Calcula o lucro total das apostas em casa agrupadas por liga e temporada
        profit_ov15ft_by_league_season = filtered_df.groupby(['League', 'Season'])['profit_over15'].sum()

        # Filtra as ligas que lucraram pelo menos 1 unidade em todas as temporadas
        profitable_leagues = profit_ov15ft_by_league_season.groupby('League').filter(lambda x: (x >= 2).all())

        # Converte o resultado filtrado em um DataFrame
        filtered_df = profitable_leagues.reset_index()

        # Cria uma tabela dinâmica para organizar os dados
        pivot_table = filtered_df.pivot_table(index='League', columns='Season', values='profit_over15', aggfunc='sum')
     ...
     #####################################################
        ...
        # Calcula o lucro total das apostas em casa agrupadas por liga e temporada
        profit_u15ft_by_league_season = filtered_df.groupby(['League', 'Season'])['profit_under15'].sum()

        # Filtra as ligas que lucraram pelo menos 1 unidade em todas as temporadas
        profitable_leagues = profit_u15ft_by_league_season.groupby('League').filter(lambda x: (x >= 2).all())

        # Converte o resultado filtrado em um DataFrame
        filtered_df = profitable_leagues.reset_index()

        # Cria uma tabela dinâmica para organizar os dados
        pivot_table = filtered_df.pivot_table(index='League', columns='Season', values='profit_under15', aggfunc='sum')
