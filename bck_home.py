...
def bck_home_page():
    ...
    with tab0:
        ...
        with col4:
            ...
            Placar_HT_A_min = st.number_input("HT_Goals_A Mínimo", value=0.0)
            Placar_HT_A_max = st.number_input("HT_Goals_A Máximo", value=10.0)
...
##### Calculo Win/Loss Lay 2x2 ####

        # Create a new DataFrame for the "Lay 2x2" table
        df_lay_22 = pd.DataFrame(columns=["Win", "Loss", "Odd Justa"])

        # Calculate the number of "Win" and "Loss" occurrences
        num_win = len(filtered_df[(filtered_df["FT_Goals_H"] != 2) | (filtered_df["FT_Goals_A"] != 2)])
        num_loss = len(filtered_df[(filtered_df["FT_Goals_H"] == 2) & (filtered_df["FT_Goals_A"] == 2)])
        total_games = num_win + num_loss
        ...
        #### Add the data to the "Lay 0x2" table ####
        df_lay_22.loc[0] = [f"{win_percentage:.2f}%", f"{loss_percentage:.2f}%", fair_odd]

        # Display the "Lay 2x2" table
        st.subheader("Lay 2x2")
        st.dataframe(df_lay_22)
    ...
    # Verificar se o DataFrame não está vazio
        if not filtered_df.empty:
    # Somar os valores da coluna 'profit_home' para obter o lucro total
            lucro_total = filtered_df['profit_Lay_2x2'].sum()
